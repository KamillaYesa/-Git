...
В Git файлы имеют четыре статуса:

* **Untracked** - файл не отслеживается Git. Он существует в рабочей копии, но не был добавлен в индекс.
* **Modified** - файл отслеживается Git и был изменен с момента последнего коммита.
* **Staged** - файл отслеживается Git и был добавлен в индекс.
* **Committed** - файл отслеживается Git и был зафиксирован в коммите.

**Между состояниями файлы могут перемещаться в следующих направлениях:**

* **Untracked → Modified:** Файл был создан или изменен в рабочей копии.
* **Modified → Staged:** Файл был добавлен в индекс.
* **Staged → Committed:** Файл был зафиксирован в коммите.
* **Committed → Modified:** Файл был изменен в рабочей копии.
* **Modified → Untracked:** Файл был удален из рабочей копии.
...
***Между состояниями файлы могут перемещаться вручную или автоматически.***

* **Ручное перемещение:**
   * Чтобы добавить файл в индекс, используйте команду `git add`.
   * Чтобы удалить файл из индекса, используйте команду `git reset`.
   * Чтобы зафиксировать изменения в коммите, используйте команду `git commit`.

* **Автоматическое перемещение:**
   * Файлы, которые были изменены в рабочей копии, автоматически перемещаются в состояние `Modified`.
   * Файлы, которые были добавлены в индекс, автоматически перемещаются в состояние `Staged`.
   * Файлы, которые были зафиксированы в коммите, автоматически перемещаются в состояние `Committed`.
  
  

**Примеры:**

* **Файл создается в рабочей копии:**

```
Untracked → Modified
```

* **Файл изменяется в рабочей копии:**

```
Modified → Modified
```

* **Файл добавляется в индекс:**

```
Modified → Staged
```

* **Файл фиксируется в коммите:**
